...
export class StoryService {
  ...
  async findStoriesByProjectId(projectId: string): Promise<StoryDto[]> {
    try {
      const existingProject: Project = await this.projectModel
        .findById(projectId)
        .exec();
      if (!existingProject) {
      ...
      return await this.storyModel
        .find({ project: existingProject._id })
        .populate('project')
        .exec();
    } catch (error) {
  ...
  async createStory(incomingStory: StoryDto): Promise<StoryDto> {
    try {
      ...
      const persistedStory = await newStory.save();
  ...
  async deleteStory(storyId: string) {
    const existingStory: StoryDto = await this.storyModel
      .findById(storyId)
      .exec();
    if (!existingStory) {
      throw new NotFoundException(`No Story found with the id ${storyId}`);
