...
describe('ProjectController', () => {
  ...
  it('CREATE PROJECT - should create a project in database if doesnot exists', async () => {
    ...
    const resposne: ProjectDto = await projectController.createProject(
      newProject,
    );
  ...
  it('CREATE PROJECT - should throw 422 error when project name already exists in DB', async () => {
    ...
    await expect(projectController.createProject(newProject)).rejects.toThrow(
  ...
  it('FIND BY ID - should return the project details when Id is passed', async () => {
    ...
    const existingProject: ProjectDto = await projectController.findProjectById(
      id,
    );
  ...
  it('FIND BY ID - should throw not found exception when a project id is not found in DB', async () => {
    ...
    await expect(
      projectController.findProjectById(invalidProjectId),
    ).rejects.toThrow(
  ...
  it('DELETE PROJECT - should throw error, when an id of INCORRECT format is sent', async () => {
    ...
    await expect(projectController.deleteProject(dummyId)).rejects.toThrow(
  ...
  it('DELETE PROJECT - should throw not found exception when a project id is not found in DB', async () => {
    ...
    await expect(
      projectController.deleteProject(invalidProjectId),
    ).rejects.toThrow(
      new NotFoundException(`No Project found with the id ${invalidProjectId}`),
    );
  });
