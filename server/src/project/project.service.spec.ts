...
describe('ProjectService', () => {
  ...
  it('CREATE PROJECT - should throw error if the name of the project is already present in database', async () => {
    ...
    await expect(projectService.createProject(incomingProject))
      .rejects.toThrow(new UnprocessableEntityException('Project with name project1 already exists'));
  ...
  it('FIND BY ID - should throw not found exception when a project id is not found in DB', async () => {
    ...
    await expect(projectService.findProjectById(invalidProjectId))
      .rejects
  ...
  it('DELETE PROJECT - should throw error, when an id of INCORRECT format is sent', async () => {
    ...
    await expect(projectService.deleteProject(dummyId))
      .rejects
  ...
  it('DELETE PROJECT - should throw not found exception when a project id is not found in DB', async () => {
    ...
    await expect(projectService.deleteProject(invalidProjectId))
      .rejects
      .toThrow(new NotFoundException(`No Project found with the id ${invalidProjectId}`));
  });
